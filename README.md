...
- 新增 `html-exporter` 末端导出器：在最终 Markdown 定稿后，用户可以额外拿到一份可直接打开和复制的 `.html` 文件
- 保留 `_clean.txt` 作为默认纯文本出口：一个适合直接复制发布，一个适合保留排版结构
- 收紧 Stage 12.5 契约、运行态记录和回归测试，让终稿导出不再只是 prompt 口头约束

如果你关心的是“最后到底能交付什么”，这一版的答案更明确了：**正文、纯净终稿、排版 HTML，三条线都能落地。**
