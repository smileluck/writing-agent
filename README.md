...
如果你只想知道这一版到底更新了什么，看上面这 3 条就够了。更老的版本记录应该去 `CHANGELOG` 或 Releases，不应该堵在首页前面。
...
- 最后输出 `_clean.txt` 终稿，并复盘这次修改学到了什么
...
3. 完整版一定要在项目根目录启动 Claude Code，最后真正交付的是 `_clean.txt` 终稿，不只是中间草稿。
